#!/usr/bin/env python3
"""Managed connector for frozen AI-BOT lineage evidence snapshots."""
...
def valid_url(value: object) -> str:
    ...
    if parsed is None or parsed.scheme != "http" or parsed.hostname not in {"127.0.0.1", "::1", "localhost"} or parsed.port != 8793 or parsed.path not in {"", "/"} or parsed.query or parsed.fragment:
        raise ConnectorError("INVALID_CONFIGURATION")
...
class Transport:
    def __init__(self, base_url: str, token: str):
        self.base_url = valid_url(base_url); self.token = token; self.opener = build_opener(HTTPHandler())
...
def execute(request: Mapping[str, Any], credentials: Mapping[str, str], transport: Any | None = None) -> list[dict[str, Any]]:
    ...
    if not isinstance(settings, dict) or set(settings) != {"api_base_url", "algorithm_key"}:
        raise ConnectorError("INVALID_CONFIGURATION")
    algorithm = settings.get("algorithm_key")
    if not isinstance(algorithm, str) or not algorithm or len(algorithm) > 64:
        raise ConnectorError("INVALID_CONFIGURATION")
    if not isinstance(settings["api_base_url"], str) or not settings["api_base_url"]:
        raise ConnectorError("INVALID_CONFIGURATION")
    ...
    if operation == "discover":
        ...
        return [
            {"protocol": PROTOCOL, "request_id": request_id, "seq": 1, "type": "resource", "resource": {"id": algorithm, "name": f"{algorithm} lineage", "type": "model_lineage", "selectable": True}},
            {"protocol": PROTOCOL, "request_id": request_id, "seq": 2, "type": "complete", "complete": {"resources_emitted": 1, "items_emitted": 0}},
        ]
    if request.get("resource_id") != algorithm or not isinstance(request.get("limit"), int) or not 1 <= request["limit"] <= MAX_LIMIT:
        raise ConnectorError("INVALID_CONFIGURATION")
    cursor = request.get("cursor") or {}
    if not isinstance(cursor, dict) or set(cursor) - {"snapshot_id", "membership_digest", "api_cursor", "total", "emitted"}:
        raise ConnectorError("INVALID_CONFIGURATION")
    snapshot = cursor or client.request("POST", snapshot_path)
    extra = {"limit": request["limit"]}
    ...
    if page.get("stream_kind") != STREAM or page.get("algorithm_key") != algorithm or page.get("membership_digest") != snapshot.get("membership_digest") or page.get("total") != snapshot.get("total"):
        raise ConnectorError("REMOTE_UNAVAILABLE")
    events = []
    for item in page.get("items", []):
        record = item.get("record") if isinstance(item, dict) else None
        payload = canonical(record)
        if not isinstance(record, dict) or record.get("schema_version") != SCHEMA or record.get("algorithm_key") != algorithm or hashlib.sha256(payload).hexdigest() != item.get("record_digest"):
            raise ConnectorError("REMOTE_UNAVAILABLE")
        record_id = record.get("record_id")
        if not isinstance(record_id, str): raise ConnectorError("REMOTE_UNAVAILABLE")
        events.append({"protocol": PROTOCOL, "request_id": request_id, "seq": len(events) + 1, "type": "item", "item": {"external_id": f"lineage:{algorithm}:{record_id}", "title": f"{algorithm} {record['payload']['kind']}", "content_type": "application/json", "content_base64": base64.b64encode(payload).decode("ascii"), "metadata": {"source_locator": f"aibot-lineage://{algorithm}/{record_id}", "record_digest": item["record_digest"]}}})
    emitted = int(snapshot.get("emitted", 0)) + len(events); next_api = page.get("next_cursor") or ""
    complete: dict[str, Any] = {"resources_emitted": 0, "items_emitted": len(events)}
    if next_api:
        complete["next_cursor"] = {"snapshot_id": snapshot["snapshot_id"], "membership_digest": snapshot["membership_digest"], "api_cursor": next_api, "total": snapshot["total"], "emitted": emitted}
    elif emitted != int(snapshot["total"]):
        raise ConnectorError("REMOTE_UNAVAILABLE")
    events.append({"protocol": PROTOCOL, "request_id": request_id, "seq": len(events) + 1, "type": "complete", "complete": complete})
    return events
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"protocol": PROTOCOL, "request_id": request_id, "seq": 1, "type": "error", "error": {"code": exc.code, "retryable": exc.retryable}}, separators=(",", ":")))
        return 1
